...
export default function FunFacts({ isDark = true }: { isDark?: boolean }) {
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-2">
        {funFacts.map((fact, i) => (
          <div key={i} className={`${card} border rounded-lg p-3 text-center`}>
            ...
            <p className={`${body} font-inter text-xs leading-relaxed`}>{fact.text}</p>
          </div>
        ))}
      </div>
      <div className="flex items-center gap-3 pt-2">
        <span className={`${muted} font-inter text-xs`}>Connect →</span>
      ...
      <p className={`${credit} font-inter text-xs text-center pt-2`}>
        Vibe coded with Cursor & Antigravity · React + Tailwind · Deployed on GitHub Pages
      </p>
    </div>
  )
}
