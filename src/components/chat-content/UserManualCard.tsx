  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
      <div className={`${card} border rounded-lg p-4`}>
        <p className="text-emerald-400/80 font-inter text-sm font-semibold mb-2.5">✅ Do's</p>
        <ul className="space-y-2">
          {philosophy.userManual.dos.map((item, i) => (
            <li key={i} className={`flex items-start gap-2 ${body} font-inter text-sm`}>
      ...
      <div className={`${card} border rounded-lg p-4`}>
        <p className="text-amber-400/80 font-inter text-sm font-semibold mb-2.5">⚡ Notes</p>
        <ul className="space-y-2">
      ...
      <div className="md:col-span-2">
        <p className={`${isDark ? 'text-zinc-600' : 'text-gray-400'} font-mono text-sm text-center mt-1`}>
          — end of README.md —
        </p>
      </div>
    </div>
